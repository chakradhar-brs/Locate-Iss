...
const iss_location_api_endpoint =
...
iss_center.addEventListener("change", function(e) {
  center_iss = e.target.checked;
});
...
async function getISSLocation() {
  ...
  const issLocation = await fetch(iss_location_api_endpoint, {
    signal: signal
  });
...
async function updateISS(lngLat, utcTimeFormat, localTimeFormat, sp, al, vi) {
  ...
  latitude.innerHTML =
    Math.abs(lngLat[1]) + `&#176; ${getDirection(lngLat[1], "lat")}`;
  longitude.innerHTML =
    Math.abs(lngLat[0]) + `&#176; ${getDirection(lngLat[0], "lng")}`;
  const locDetails = await fetch(
    `${geoCodingUrl}/${lngLat[0]},${lngLat[1]}.json?access_token=${mapbox_accesstoken}`
  );
  const locData = await locDetails.json();
  let region_data = locData.features.filter(
    e => e.place_type[0] === "region"
  )[0];
